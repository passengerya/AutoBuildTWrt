...
6. **发布模型（按日一个 Release，天然不覆盖）**：每天自动构建发布到当天的新 Release（tag 如 `Autobuild-x86-64-2026-09-18`，当天各通道固件都进这个清单）；**手动构建每次创建独立的新 Release**（tag 加 `-HHMM`，如 `Autobuild-x86-64-2026-09-18-2034`）——历史固件全部保留，不会互相覆盖。
