...
**25.12 apk 通道（21 个）**：argon25、build-pw、mosdns25、oc25、pw2-25、ssrp25、store25、25-quickfile、25-singbox、25-openwrt-daede、25-clashoo、25-rtp2httpd、25-advancedplus、25-aurora-theme、25-aurora-config、25-oaf、25-amlogic、25-tailscale-community、25-easytier、25-shadcn、25-bandix（2026-09-24 新增，timsaya 官方 Release apk）

25.12 通道 imm 固定段补齐（2026-09-24，官方源确认）：adguardhome、dufs、homeproxy 三应用在 ImmortalWrt 25.12.1 官方源齐备（含 zh-cn 语言包，adguardhome 除外），已加入 apk-custom-packages.sh 固定段与 imm-packages.md；lucky/momo/nikki/nekobox/openlist2/高级卸载上游仍无 25.12 apk 资产，继续缺席。
...
| 41 | oaf 首启配置生成在个别设备上静默失败：脚本被正常执行器删除、`/etc/config/appfilter` 却没生成 → 菜单不显示。99-custom.sh 加「appfilter 缺失/为空则直接写文件重建」自愈（2026-09-23/24 两版迭代） | 用户工控机实测链：①上游 94_feature_3.0/95_time_daily_limit 的 heredoc 体用 **8 个空格缩进**，`<<-EOF` 只剥 tab → uci batch 收到带前导空格的命令、静默失败（-q 吞报错）且退出码 0 → 被 `( . ./file ) && rm` 删除；对照：base 脚本（tab 缩进）在该设备首启全部正常，设备本身无碍。②第一版自愈用 `uci set cfg.sec=type` 创建新配置包——**uci set 对不存在的配置包一律 Entry not found**（正常语义），这版自愈在任何设备都无效；**创建新配置包只有 batch/import/直接写文件三条路**。③最终自愈=echo 直写 /etc/config/appfilter（该设备上唯一实测有效的方式）。教训：写自愈前先验证所用机制的语义（uci set 需要配置包已存在）；上游脚本的缩进 bug 无法在 store 侧改（ipk 原样同步），固件侧兜底是正解 |
| 42 | **重打包 ipk 时外层 tar 成员名必须带 `./` 前缀且属主归 0**（2026-09-24 下游构建 #43 失败根因） | oaf.yml 修补 94 缩进时 `tar -czf out debian-binary data.tar.gz control.tar.gz` 重打包：ImageBuilder 索引生成器按 **`./control.tar.gz`** 提取成员，无前缀时读不到 control → 索引条目元数据损坏 → opkg「Cannot install package luci-app-oaf」+ i18n「found, but incompatible with architectures」（架构字段错乱），构建 Error 255。修复：`tar -czf --owner=0 --group=0 ./debian-binary ./data.tar.gz ./control.tar.gz`。**教训：重打包任何 ipk 后必须用 `tar -xzOf pkg ./control.tar.gz` 验证可提取**，且成员顺序/前缀/属主与原版逐项对齐——本地 tar 解包成功≠下游解析器能读 |
| 43 | **首启脚本要区分「全新安装」与「保留配置升级」**：`$PKG_UPGRADE=1` 由 fstools 恢复配置包（mount_root）时导出，各官方包 uci-defaults（如 30_luci-theme-argon）都以此守卫跳过升级重初始化；自研 99-custom.sh 的「无条件踩踏」项（主题 mediaurlbase）必须同样守卫（2026-09-24 用户实测升级后主题被踩回 Bootstrap）。密码类配置无脚本触碰、keep.d（base-files-essential）保留 /etc/shadow——x86 保留配置升级的密码丢失应归因设备侧 sysupgrade 配置恢复失败（fstools mount_root 从 /boot 恢复 tarball；该设备有前科，见 #36），用对照物实验（升级前 `echo marker > /etc/config/sysupgrade-marker` + 设密码，升级后查两者）判定 |
