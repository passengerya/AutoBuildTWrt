...
- **发布不覆盖**（2026-09-17 起）：上传前把固件按构建时间重命名（文件名插入 `-YYYYMMDD-HHMM` 北京时间时间戳，如 `immortalwrt-24.10.6-x86-64-generic-squashfs-combined-efi-20260917-0740.img.gz`），历史固件全部保留、一个固件对应一个文件；Release 说明中另有「构建方式 + 时间」标注。
