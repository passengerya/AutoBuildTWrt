...
# 自动生成: openwrt-daede | eBPF代理 | 基于 eBPF 的高性能透明代理(dae/daed) | 2026.09.14 上游停更(保留旧版) | 取消下一行注释即启用
#CUSTOM_PACKAGES="$CUSTOM_PACKAGES dae daed luci-app-daede vmlinux-btf"
# 自动生成: passwall | PassWall | 代理工具(自带依赖) | 26.9.9 上游停更(保留旧版) | 取消下一行注释即启用
...
# 自动生成: sing-box | Sing-box内核 | 通用代理内核 | v1.14.0 上游停更(保留旧版) | 取消下一行注释即启用
...
# ───────────────────── 系统与界面 ─────────────────────
# 自动生成: argon | Argon主题 | 简洁主题, 支持明暗自动切换 | 2.4.7 上游停更(保留旧版) | 取消下一行注释即启用
...
# 自动生成: luci-app-aurora-config | 极光配置中心 | Aurora 主题配置中心(提供 /etc/config/aurora, 与主题配套启用) | 1.2.0-r20260808 上游停更(保留旧版) | 取消下一行注释即启用
CUSTOM_PACKAGES="$CUSTOM_PACKAGES luci-app-aurora-config luci-i18n-aurora-config-zh-cn"
# 自动生成: luci-theme-aurora | 极光主题 | 极光主题界面(需配套 luci-app-aurora-config 配置中心, 会接管 LuCI 菜单/路由, 谨慎启用) | 1.3.0-r20260830 上游停更(保留旧版) | 取消下一行注释即启用
CUSTOM_PACKAGES="$CUSTOM_PACKAGES luci-theme-aurora"
# 自动生成: luci-theme-shadcn | Shadcn主题 | 现代 Shadcn 风格界面主题(会接管 LuCI 菜单/路由, 24.10 下谨慎启用) | 0.5.0-r20260830 上游停更(保留旧版) | 取消下一行注释即启用
